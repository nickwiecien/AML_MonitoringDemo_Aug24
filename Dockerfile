...
COPY ./model/conda.yaml /tmp/conda.yaml
RUN conda env create -n userenv -f /tmp/conda.yaml && \
    ...
    $CONDA_ENV_DIR/userenv/bin/pip install azureml-inference-server-http==$SERVER_VERSION azure-ai-ml==1.16.1 azureml-ai-monitoring==1.0.0 azureml-core==1.56.0
